//
//  1592. Rearrange Spaces Between Words.swift
//  Algorithm
//
//  Created by 이정호 on 10/14/24.
//
...
extension LeetCode {
    class RearrangeSpacesBetweenWords {
        func reorderSpaces(_ text: String) -> String {
            ...
            let splitedBySpace = text.split(separator: " ").map { String($0) }
            let wordsCount = splitedBySpace.count - 1
            
            guard wordsCount > 0 else {
                let spaceString = String(repeating: " ", count: spaceCount)
                let word = splitedBySpace.first ?? ""
                return word + spaceString
            }
            
            let arrangedSpaceCount = spaceCount / wordsCount
            
            let arrangedSpaceString = String(repeating: " ", count: arrangedSpaceCount)
            
            let newText = splitedBySpace.joined(separator: arrangedSpaceString)
            
            let remainSpaceCount = spaceCount % wordsCount
            guard remainSpaceCount != 0 else {
                return newText
            }
            let remainSpaceString = String(repeating: " ", count: remainSpaceCount)
            return newText + remainSpaceString
